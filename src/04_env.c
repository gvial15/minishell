...
static int	unset_var_count(char **args, char **envp)
{
	...
	while (args[++i])
		if (valid_unset(args[i], 1)
			&& already_exist(get_varname_equal(args[i]), envp, 1) >= 0)
			count++;
	return (count);
}
...
char	**unset_env_var(char **envp, char **args, t_ms *ms)
{
	...
	i = -1;
	while (envp[++i])
		if (already_exist(get_varname_equal(envp[i]), args, 1) == -1)
...
static void	reassign(char **new_envp, char **args)
{
	...
	while (args[++i])
	{
		arg_i = already_exist(get_varname_equal(args[i]), new_envp, 1);
		if (arg_i != -1)
		{
			free(new_envp[arg_i]);
...
static int	export_var_count(char **args, char **envp)
{
	...
	i = 0;
	while (args[++i])
		if (valid_export(args[i], 1) && already_exist(get_varname_equal(args[i]), envp, 1) == -1)
			count++;
	return (count);
}
...
char	**export_env_var(char **envp, char **args, t_ms *ms)
{
	...
	j = 0;
	while (args[++j])
		if (valid_export(args[j], 0) && already_exist(get_varname_equal(args[j]), new_envp, 1) == -1)
